...
public readonly record struct Selection<T>
{
	...
	public Selection(IReadOnlyList<T> contenders, double selectionPoint = 0.5)
	{
		...
		ImmutableArray<T>.Builder all = ImmutableArray.CreateBuilder<T>(len);
		ImmutableArray<T>.Builder selected = ImmutableArray.CreateBuilder<T>(middleIndex);
		ImmutableArray<T>.Builder rejected = ImmutableArray.CreateBuilder<T>(len - middleIndex);
		for (int i = 0; i < len; i++)
		{
			T? g = contenders[i];
			all[i] = g;
